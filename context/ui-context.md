...
| Role             | CSS Variable           | Hex / Value               | Tailwind Utility                |
| ---------------- | ---------------------- | ------------------------- | -------------------------------- |
| Page background  | `--bg-base`            | `#080809`                 | `bg-base`                        |
| Surface          | `--bg-surface`         | `#111114`                 | `bg-surface`                     |
| Elevated surface | `--bg-elevated`        | `#18181c`                 | `bg-elevated`                    |
| Subtle surface   | `--bg-subtle`          | `#1e1e23`                 | `bg-subtle`                      |
| Default border   | `--border-default`     | `#2a2a30`                 | `border-surface-border`          |
| Subtle border    | `--border-subtle`      | `#3a3a42`                 | `border-surface-border-subtle`   |
| Primary text     | `--text-primary`       | `#f0f0f4`                 | `text-copy-primary`              |
| Secondary text   | `--text-secondary`     | `#c0c0cc`                 | `text-copy-secondary`            |
| Muted text       | `--text-muted`         | `#808090`                 | `text-copy-muted`                |
| Faint text       | `--text-faint`         | `#505060`                 | `text-copy-faint`                |
| Brand accent     | `--accent-primary`     | `#00c8d4` (cyan)          | `text-brand` / `bg-brand`        |
| Brand dim        | `--accent-primary-dim` | `rgba(0, 200, 212, 0.12)` | `bg-accent-dim`                  |
| AI accent        | `--accent-ai`          | `#6457f9` (indigo-purple) | `bg-ai` / `text-ai`              |
| AI text          | `--accent-ai-text`     | `#8b82ff`                 | `text-ai-text`                   |
| Error            | `--state-error`        | `#ff4d4f`                 | `text-error` / `bg-error`        |
| Success          | `--state-success`      | `#34d399`                 | `text-success` / `bg-success`    |
| Warning          | `--state-warning`      | `#fbbf24`                 | `text-warning` / `bg-warning`    |

Tailwind utility names map to these variables via `@theme inline` in `app/globals.css`.

These app-level tokens are distinct from the standard shadcn/ui tokens (`--background`, `--primary`, `--card`, `--border`, etc.) that `components/ui/*` files use internally — those are also defined in `app/globals.css` and mapped onto this same dark palette so both token systems render one consistent theme. App-level components should use the tokens above; reserve shadcn's raw tokens (`bg-primary`, `text-foreground`, etc.) for use inside `components/ui/*` only.
